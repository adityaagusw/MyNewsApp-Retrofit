...
class OlahragaActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Inisialisasi Widget
        recyclerView = findViewById<View>(R.id.rvListBeritaOR) as RecyclerView?
    ...
    private fun tampilBeritaSports() {
        val api = InitRetrofit.instance
        // Siapkan request
        val beritaCall = api.beritasports()
        // Kirim request
            ...
            override fun onResponse(call: Call<ResponseBerita>, response: Response<ResponseBerita>) {
                ...
                if (response.isSuccessful) {
                    ...
                    if (status) {
                        ...
                        val adapter = AdapterBerita(this@OlahragaActivity, data_berita!!)
                        recyclerView!!.adapter = adapter
                    } else {
                        // kalau tidak true
                        Toast.makeText(this@OlahragaActivity, "Tidak ada berita untuk saat ini", Toast.LENGTH_SHORT).show()
                    }
                }
            }
